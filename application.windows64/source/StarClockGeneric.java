...
public class StarClockGeneric extends PApplet {
...
//import java.time.*;
    ...
    double GHA0 = 0;
    double GHA = Math.toRadians(-214.21545f); //(-250.297);//(-214.21545); //(-250.297);//(-214.21545); //(34.21545);  //<---- = 214 + 180 -360 //(214.21545); //Arcturus
     double dec = Math.toRadians(-19.335f);
    double GHA1 = Math.toRadians(-279.2346f); //(-315.568);//(-279.2346); //(-315.568);//(-279.2346); //(99.2346);//(279.2346); //Vega
    double dec1 = Math.toRadians(-38.758f);
    double GHA2 = Math.toRadians(-165.932325f);//(-202.375);//(-165.932325);//(-202.375);//(-165.932325);//(14.067675);//(165.932325); //(345.932325); //(165.932325); //Dubhe
    double dec2 = Math.toRadians(-61.909f);
    double alt = (55.53f);//(89.50);//(55.53);
    double alt1 = (64.648f);//(89.50);//(64.648);
    double alt2 = (38.18f);//(89.50);//(38.18);
...
public void draw() {
 ...
 //   Data = MainActivity.MyHandler.getData();
  ...
  //  float gha = Data[2];
float gha = 150.0f;
    if (gha > 0.0f && gha < 180.0f) {
        gha = (float)GHA; // 150.0; /// Data[2];

    }else{
        gha = 360 + 150.0f; // Data[2];
...
// println(Mz[2][0] + " " + Mz[2][1] + " " + Mz[2][2]);
     ...
     //    GHA2 = -202.375;
        ...
        for( double L0 = -180.0f; L0 <= 180.0f; L0 += .1f )
        {
           ...
           Mz = Rz(((double)(abs((float)(Math.toRadians(360.0f) - GHA)))), Mz);
...
// println(Mz[2][0] + " " + Mz[2][1] + " " + Mz[2][2]);
            ...
            //   float alt = map(mouseX,width-width,width,0,90);
            vv =  VectorSpherical2Cartesian(Math.toRadians(-39.4f),Math.toRadians(L0) );
            //   vv =  VectorSpherical2Cartesian(alt,Math.toRadians(L0) );
...
// println(GAST, "   ",LMST); //12:48 local = 10.000 GAST
      rotateY(radians( ( (float)(GAST)*15))+(77.4f)) ; //+(77.4)) ;
...
// println(res[0] + " " + res[1] + " " + res[0]);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "StarClockGeneric" };
    if (passedArgs != null) {
